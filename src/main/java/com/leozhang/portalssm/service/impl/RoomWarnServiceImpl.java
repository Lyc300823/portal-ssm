...
public class RoomWarnServiceImpl implements RoomWarnService {
    @Autowired
    RoomWarnMapper roomWarnMapper;

    @Override
    public Result selectListForPage(int pno, int psize, String sortField, String
            sortType) {
        ...
        List<RoomWarn> list =
                roomWarnMapper.selectRoomWarnListByExample(roomWarnExample);
        return Result.end(200,list,"查询成功",p.getTotal());
    }

    @Override
    public RoomWarn getRoomWarnByRoomId(Long roomId) {
        //根据roomId在数据表中检索是否存在该机房的配置，如果没有就创建⼀条记录
        RoomWarnExample roomWarnExample = new RoomWarnExample();
